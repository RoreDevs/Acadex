...
// register with made-up index numbers. The expected format is never
// revealed to the user.
function getProgramIndexPattern(program?: { name: string; code: string } | null) {
  const text = `${program?.name || ''} ${program?.code || ''}`.toUpperCase();
  if (text.includes('BTECH')) return /^B\d{9}$/i;
  if (text.includes('HND')) return /^[A-Z0-9]{2}\/20\d{2}\/\d{4}D$/i;
  return null;
}
...
export function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    setLoading(true);
    const pattern = getProgramIndexPattern(programs.find((p) => p.id === data.program));
    const indexNumber = data.index_number.trim();
    if (pattern && !pattern.test(indexNumber)) {
      setError('index_number', { type: 'pattern', message: 'Invalid index number.' });
      setLoading(false);
      return;
    }
